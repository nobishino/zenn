...
前章では、happens-before関係が**sequenced before**と**synchronized before**という2つの関係の推移的閉包としてフォーマルに定義されることを学びました。同一goroutine内の順序がsequenced before、**同期演算**と呼ばれる特定のペアがgoroutineをまたいで作る順序がsynchronized beforeでした。しかし前章では、「具体的にどんな同期演算が、どんなsynchronized before関係を作るのか」については深入りしませんでした。
...
Go Playgroundはtxtar形式で複数ファイルを書けるので、`go.mod`と複数パッケージを使って、引用の状況——パッケージp（ここではmainパッケージ）がパッケージqをimportする——をそのまま再現できます。
...
func initP() string {
	fmt.Println("3: pのパッケージ変数の初期化")
	return "p"
}

func init() {
	fmt.Println("4: pのinit関数")
}

func main() {
	fmt.Println("5: main.main, q.V =", q.V)
}
-- go.mod --
module play.ground
-- q/q.go --
...
func initQ() string {
	fmt.Println("1: qのパッケージ変数の初期化")
	return "hello"
}

func init() {
	fmt.Println("2: qのinit関数")
}
```

👉 [Go Playgroundで実行する](https://go.dev/play/p/OohprWbWBD3)

何度実行しても、必ず`1`→`5`の順、つまり「qのパッケージ変数の初期化 → qのinit関数 → pのパッケージ変数の初期化 → pのinit関数 → `main.main`」の順に出力されます。importされる側（q）の初期化が、importする側（p）の初期化より必ず先に完了していることが確認できます。
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/x9U0qqWujwc)
...
つまり、「goroutineが終わったこと」自体は、他のどんな演算とも矢印で結ばれません。後述のプログラム例の`hello`関数をグラフにすると、次のようになります。

```mermaid
graph TD
    subgraph M["helloを実行するgoroutine"]
        GO["go func()..."] --> PR["print(a)"]
    end
    subgraph G["起動されたgoroutine"]
        A["a = 'hello'"]
    end
    GO --> A
```
*`go`文から起動されたgoroutineへの矢印（前節のルール）はあるが、goroutineの終了から出ていく矢印は存在しない。よって`[a = 'hello']`と`[print(a)]`は、第3章ルール2で見たS→X/Y型そのもの、つまり並行である*

`[a = 'hello']`と`[print(a)]`が並行なので、観測可能性の条件2により、`print(a)`は書き込みを観測することも、しないこともあり得ます。「観測する保証」はどこにもありません。
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/FPpEkgg_bn3)
...
> チャネルへの送信は、そのチャネルからの対応する受信の完了よりsynchronized beforeである。
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/hRjXeFDpniK)
...
> チャネルのcloseは、チャネルがcloseされたことによってゼロ値を返す受信よりsynchronized beforeである。

```mermaid
graph LR
    C["close(c)"] ==> R["<-c（ゼロ値を受信）"]
```
*closeは、closeによってゼロ値を返す受信よりsynchronized beforeである*
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/6g9njwU5Y7H)
...
> バッファなしチャネルからの受信は、そのチャネルへの対応する送信の完了よりsynchronized beforeである。
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/KjWqYddWG2L)
...
> 容量Cのチャネルに対するk番目の受信は、そのチャネルへの(k+C)番目の送信の完了よりsynchronized beforeである。

このルールは、言い換えると「**(k+C)番目のsendは、k番目のreceiveが完了するまで完了できない**」ということです。容量`C`のチャネルには値が`C`個までしか入らないので、(k+C)番目のsendが完了できるのは、それまでに`k`個以上の値が取り出された後、つまりk番目のreceiveが完了した後に限られるわけです。

これが、容量`C`のバッファ付きチャネルを「カウンティングセマフォ」として使うときの根拠になるルールです。

```mermaid
graph LR
    RK["k番目のreceive"] ==> SKC["(k+C)番目のsend"]
```
*k番目のreceiveは、(k+C)番目のsendの完了よりsynchronized beforeである*
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/eq0KItEfkY6)

各goroutineはタスクの前に`limit <- 1`で送信し、タスクが終わったら`<-limit`で受信します。`limit`の容量は3なので、上のルールにより、4番目のsend（4つ目に動き出したgoroutineの`limit <- 1`）は1番目のreceive（どれかのタスクの完了）が済むまで完了できません。こうして、同時に実行されるタスクが最大3つに制限されます。どのタスクがどの順番で終わるかは実行のたびに変わりますが（並行に走っているので）、5つ全てが必ず出力されます。
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/HCaYrQD3YE0)
...
👉 [Go Playgroundで実行する](https://go.dev/play/p/B8i_dTYr8PW)
