---
title: "[Go Quiz] 単一の型のみによる型制約を持つ型パラメータにその型の変数を代入できるか"
emoji: "🐈"
...
func F[T int]() T { 
	x := 1 // xを短縮変数宣言(short variable declaration)する。右辺はuntyped constantの1なのでそのdefault typeであるintがxの型となる。
	var t T // T型の変数を宣言する。
...
単一の型のみによる型制約を持つ型パラメータにその型の変数を代入できるのは、その型がnamed typeではないときであり、またそのときに限る。
