...
**最終更新時のGoバージョン: Go1.27(2026-08-19)**
...
- 組み込みの`min, max`関数が追加されたことに対して記述が対応していない
...
- 「関数およびメソッド」と「型」は「型パラメータ」を持つことができる。
- 「型パラメータ」の満たすべき性質は「インタフェース型」を「型制約」として使うことで表す。

このことを、3つの基本的な具体例で見ていきましょう。

- 型パラメータを持つ関数`f[T Stringer]`
- 型パラメータを持つ型`Stack[T any]`
- 型パラメータを持つ型`Set[T comparable]`
...
### メソッド自身も型パラメータを持つことができる

ここまでに出てきた`Push`や`Pop`で使われている型パラメータ`T`は、メソッド自身が新たに宣言したものではありません。`Stack[T]`というジェネリック型のメソッドを宣言するとき、レシーバ`*Stack[T]`において型パラメータ`T`が宣言され、その`T`をメソッドのシグネチャや実装の中で利用しています。

Go1.27からは、これに加えて**メソッド自身が新しい型パラメータを宣言する**こともできるようになりました。このようなメソッドをジェネリックメソッド(generic method)と呼びます。
...
ジェネリックメソッドによって、このような処理をパッケージ全体の名前空間ではなく、特定の型に属するAPIとして定義できるようになりました。

https://github.com/golang/go/issues/77273

:::message
インタフェースのメソッドは型パラメータを宣言できません。また、ジェネリックメソッドによってインタフェースのメソッドを実装することもできません。
:::
...
- 「関数およびメソッド」と「型」は「型パラメータ」を持つことができる。
- 「型パラメータ」の満たすべき性質は「インタフェース型」を「型制約」として使うことで表す。
- 関数・メソッド・型の宣言に`[T constraint]`という文法要素をつけると、「型パラメータTを宣言する。Tは`constraint`を満たさなければならない」という意味になる。`constraint`は型制約と呼ばれ、インタフェース型を用いる。
- ジェネリック型のメソッドではレシーバから型パラメータを受け取って利用でき、Go1.27からはそれとは別にメソッド自身の型パラメータも宣言できる。
...
| 用語 | 意味 |
| ---- | ---- |
| 型パラメータ | 関数宣言・メソッド宣言・型宣言の時点では定まっていない型に対するプレースホルダーとして働く型のこと。 |
| ジェネリック関数 | 型パラメータ付きで宣言された関数のこと。 |
| ジェネリックメソッド | メソッド自身の型パラメータ付きで宣言されたメソッドのこと。 |
| ジェネリック型 | 型パラメータ付きで宣言された型のこと。| 
| 型制約 | 型パラメータを宣言するときに必ずセットで指定しなければいけないインタフェース型のこと。型制約によって、(1)その型パラメータに対して**実際に渡せる具体的な型の範囲**と、(2)型パラメータで表される抽象的な型の値に対して**できる操作の範囲**が決まる。|
...
## 標準ライブラリのジェネリックメソッド `(*rand.Rand).N`

ここまで説明したジェネリックメソッドと`~`の両方が使われている標準ライブラリの例として、Go1.27で`math/rand/v2`の`Rand`型に追加された`N`メソッドがあります。
...
`intType`は、`~int`, `~int8`, `~int16`, ... のように、すべての符号付き・符号なし整数型をunderlying typeとして持つ型を許す型制約です。つまり、単に`int`や`int64`などを渡せるだけでなく、それらをunderlying typeに持つdefined typeも`Int`として使えます。
...
もともと`math/rand/v2`には同じ型制約を使うパッケージレベルのジェネリック関数`rand.N`がありました。一方で`Rand`の特定の乱数源を使う同等のAPIをメソッドとして定義することは、Go1.26まではメソッド自身が型パラメータを宣言できなかったため不可能でした。Go1.27のジェネリックメソッドによって、`Rand.N`を`Rand`型に属するAPIとして追加できるようになったわけです。

https://pkg.go.dev/math/rand/v2#Rand.N
...
Goジェネリクスで「できそうなのにできないこと」がいくつかあります。以前は **「メソッドにおいて独自の型パラメータを追加できない」** こともその代表例でしたが、この制限はGo1.27で解消されました。

この章では、現在も残っている「できそうでできないこと」を説明しましょう。ただ単純に列挙しても良いのですが、より整理して理解するために、 **Goのジェネリクスで「できそうなこと」とはそもそも何なのかを考えてみます。**
...
もしも、この文を一般化した **「型制約を満たすすべての型について操作Xが可能ならば、型パラメータ`T`に対しても操作Xが可能である」** というテーゼが成り立つなら非常に分かりやすく、ある意味で理想的です。Goのジェネリクスに対して「できそうなこと」だとプログラマーが期待することの多くは、このテーゼが成り立って欲しいという期待に基づいていると思います。そこでこの記事では、これをジェネリクスの **「理想のテーゼ」** と呼ぶことにします。

しかし、2026年8月(Go1.27)現在、この「理想のテーゼ」は必ずしも成り立ちません。"操作X"に様々なものを当てはめて、それをみていきましょう。


:::message
Goの公式ブログ https://go.dev/blog/coretypes においても、元々のジェネリクスの設計思想は次のようなものであった、と書かれています。

> an operation involving operands of generic type should be valid if it is valid for any type permitted by the respective type constraint. To simplify matters with respect to the implementation, knowing that we would be able to relax rules later, this approach was not chosen universally. 

拙訳:

> ジェネリックな型のオペランドに係る演算は、その演算が、対応する型制約によって許される全ての型に対して有効ならば、有効であるべきだ。
> (訳註: ジェネリクスという言語機能の)実装についての問題を単純化するため、また、(訳註: 言語機能の)制約は後から緩めることができるだろうとわかっているので、このアプローチは全部の機能に対しては選ばれなかった。
:::

次のセクションから、型パラメータ`T`は型制約`Constaraint`を満たすものとします。そして、`T`の値について考えられるさまざまな操作Xを、
...
:::message
入門段階では細かすぎる仕様だと思うので、気になったときに参照する程度でご利用いただければ良いと思います。
:::

:::message
**言い訳コーナー**

実はこの分類方法論は完全に客観的方法とは言えず、同じ言語機能に対しても操作Xの定義に恣意性があって、それによって「理想のテーゼ」が成り立つかどうかが変わったりします。（興味ある方は「インデックス式」などで考えてみてください）

ただ、バラバラにただ列挙するよりはだいぶ見通しが良くなる方法だと思ったので、強行しました。

操作Xの定義によってどちらに分類されるかが変わる言語機能については、Goジェネリクスに不慣れな人が「これはできるかも」と期待しそうかどうかによって分類結果を決めました。
...
次のそれぞれの「操作X」については、 **「型制約を満たすすべての型について操作Xが可能ならば、型パラメータ`T`に対しても操作Xが可能である」** というテーゼが成り立ちます。
...
次のそれぞれの「操作X」については、 **「型制約を満たすすべての型について操作Xが可能ならば、型パラメータ`T`に対しても操作Xが可能である」** というテーゼが **必ずしも成り立ちません。**
...
型パラメータ`F`の型制約`Constraint`を満たすすべての型について、その型が関数型であり、特定の引数`(a)`に対して関数呼び出しが可能だとしても、型パラメータ`F`の値である関数についてその呼び出しが可能だとは限りません。
...
型制約`Constraint`を満たすすべての型について、その型の値vを使って`for _,e := range v {...}`という`for`文によって型`E`の値`e`を取り出せるしても、型パラメータ`T`の値に対して同様の操作ができるとは限りません。
