...
フェーズ3（期待結果の指定と照合）、フェーズ4（CI、`file=` バックエンド）。
...
| `expect=test` | 実装済 | `file=` のパッケージに `go test` を回す |
...
| `file=gosample/main.go` | 実装済 | リポジトリ内の .go ファイルを正とする。下記 |
...
**検証を通らないブロックに手書きのリンクが付いている場合は、`verify` が
「検証されていないリンク」として報告する。** そのリンクが記事の言うとおりの
ものかをツールは確かめようがないので、ブロックを直すか、意図的なリンクなら
`playground=keep` で明示する。抜粋だけを本文に載せてリンク先には完全版を置く、
という書き方（`learn_go_quine_for_great_good.md` の embed 版 Quine、
`5.synchronization.md` の複数パッケージ例など）が後者にあたる。

### 1ファイルに収まらないサンプル (`file=`)

複数ファイルに分かれたプログラム、`go test` で確かめたいもの、ビルドタグが要る
ものは、本文から復元できない。こういうサンプルはリポジトリに実際の Go コードとして
置き、ブロックはその写しを載せる。

```markdown
<!-- zenncode: file=gosample/multi/main.go expect=run -->
```

- **ファイルが正** — `fix` がファイルの内容をブロックに流し込み、`verify` は
  両者が食い違っていたら失敗する（`this block no longer matches ...`）。
  本文だけ直してファイルを直し忘れる、の逆も起きない
- **パッケージごとビルドする** — そのディレクトリの `go.mod` で `go build` するので、
  兄弟ファイル・ビルドタグ・依存がそのまま効く。`expect=test` なら `go test`、
  `expect=run` なら `go build` してから実行する
- **Playground リンクは付けない** — 1ファイルに収まらないから `file=` を使うわけで、
  Playground には載らない
- `imports=` と `goversion=` は使えない（モジュールが決めることなので）

`GOPROXY=off` は `file=` でも同じなので、サンプル用モジュールの依存は
標準ライブラリだけ（あるいはモジュールキャッシュにある状態）である必要がある。
...
165個中53個が失敗している。フェーズ3の前は75個で、意図的にコンパイルエラーに
なるサンプルを `expect=compile-error` に移したぶんと、wasm のサンプルを
`goos=`/`goarch=` で検証できるようにしたぶんが減った。残りの内訳は
